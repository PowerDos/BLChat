...
public class FriendTableDao {
    ...
    // 获取好友信息
    public FriendEntity getFriendInforByFriendId(int friendId){
        //创建数据库
        ...
        if (cursor.moveToNext()) {
            ...
            friendEntity.setFriendName(cursor.getString(cursor.getColumnIndex(FRIEND_NAME)));
    ...
    public List<List<Map<String, Object>>> getGrouopChildList(List<String> parentList) {
        ...
        for(int i = 0;i < parentList.size();i++) {
            ...
            while (cursor.moveToNext()) {
                map.put(FRIEND_ID, cursor.getString(cursor.getColumnIndex(FRIEND_ID)));
