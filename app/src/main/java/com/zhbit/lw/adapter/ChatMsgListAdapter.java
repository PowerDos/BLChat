...
public class ChatMsgListAdapter extends BaseAdapter implements View.OnClickListener, View.OnLongClickListener{
    ...
    public int getCount() {
        return chatEntity.getChatMsgData().size();
    }

    @Override
    public Object getItem(int position) {
        return chatEntity.getChatMsgData().get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        // 判断消息是发送还是接受
        String flag = chatEntity.getChatMsgData().get(position).get(CHAT_MSG_TYPE).toString();
        if (flag.equals(CHAT_MSG_TYPE_SEND)) {
            ...
            tvContent.setText(chatEntity.getChatMsgData().get(position).get(CHAT_MSG_CONTENT).toString());
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.leftMsg_userHead:
                ...
                intent.putExtra(USER_ID, chatEntity.getUserId());
                intent.putExtra(FRIEND_ID, chatEntity.getFriendId());
